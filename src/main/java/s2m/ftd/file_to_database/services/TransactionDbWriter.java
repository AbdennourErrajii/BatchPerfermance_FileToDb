...
import s2m.ftd.file_to_database.customs.CustomJdbcItemWriter;
...
@Slf4j
public class TransactionDbWriter extends CustomJdbcItemWriter<Transaction> {

    public TransactionDbWriter(DataSource dataSource) {
        super(dataSource, "INSERT INTO transaction (" +
        ...
        setItemPreparedStatementSetter(new ItemPreparedStatementSetter<Transaction>() {
            @Override
        ...
        setAssertUpdates(true);
        try {
            afterPropertiesSet();
        } catch (Exception e) {
            log.error("Failed to initialize TransactionDbWriter", e);
